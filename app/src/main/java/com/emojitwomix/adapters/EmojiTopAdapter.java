...
import static com.emojitwomix.activities.MainActivity.APITOP;
import static com.emojitwomix.activities.MainActivity.FRAME_DELAY_MS;
...
import com.emojitwomix.activities.MainActivity;
...
public class EmojiTopAdapter extends RecyclerView.Adapter<EmojiTopAdapter.ViewHolder> {
    ...
    private void actualizarEstadoUI(EmojiTopAdapter.ViewHolder holder, boolean isSaved) {
        Context contexto = holder.itemView.getContext();
        if (isSaved) {
            holder.guardar.setEnabled(false);
            holder.guardar.setColorFilter(ContextCompat.getColor(contexto, R.color.gris));
            holder.fondodescarga.setBackgroundColor(ContextCompat.getColor(contexto, R.color.teal_200));
            holder.idemoji.setTextColor(ContextCompat.getColor(contexto, R.color.teal_700));
        } else {
            holder.guardar.setEnabled(true);
            holder.guardar.setColorFilter(ContextCompat.getColor(contexto, R.color.purple_1));
            holder.fondodescarga.setBackgroundColor(ContextCompat.getColor(contexto, R.color.purple_0));
            holder.idemoji.setTextColor(ContextCompat.getColor(contexto, R.color.purple_1));
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        if (data.get(position).containsKey("isSaved")) {
            Object savedValue = data.get(position).get("isSaved");
            if (savedValue instanceof Boolean) {
                isSaved = (Boolean) savedValue;
            }
        }
        actualizarEstadoUI(holder, isSaved);
      ...
      //  holder.femoji1.setImageURI(Uri.parse(API_formado+emoji_formado));
        ...
        holder.guardar.setOnClickListener(v -> {
         //   captureMultipleImages(holder.layoutEmojiCreation);
            data.get(position).put("isSaved", true);

            // Actualizar la interfaz de usuario para reflejar el cambio
            actualizarEstadoUI(holder, true);
        ...
        if (Objects.equals(tipo, "manual")) {
            ...
            LottieCompositionFactory.fromUrl(context, String.valueOf(Uri.parse(API_manual+base)))
                    .addListener(composition -> {
                        ...
                        holder.mixedEmoji.setVisibility(View.INVISIBLE);
                        holder.mixedEmoji.setAnimation(R.raw.vacio);
                    });
        }
        else {
...
//                        Careta version
                    ...
                    int ancho4 = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, ancho, contexto.getResources().getDisplayMetrics());
                ...
                holder.posicionemoji.setRotation(10);
...
//                        posicioncara.startAnimation(customAnimation);
 ...
 //                mixedEmojiobjetos.setImageURI(Uri.parse(objetos));
        ...
        if (ojos_objetos != null) {
            ...
            if (Objects.equals(emoji1, 26) || Objects.equals(emoji2, 26) || Objects.equals(emoji1, 99) || Objects.equals(emoji2, 99)) {
            holder.mixedEmojimanos2.setVisibility(View.VISIBLE);
            holder.mixedEmojimanos2.setImageURI(Uri.parse(API_formas+manos));
            holder.mixedEmojimanos.setVisibility(View.GONE);

                Log.e("LottieError", "aki manos entro"+emoji1);
            ...
            Log.e("LottieError", "aki manos."+emoji1);
...
// Luego, en tu código donde actualizas el progreso y el texto:
            contexto.runOnUiThread(() -> {
              ...
              //  int progress = (currentFrame * NUM_FRAMES) / 50; // Suponiendo que tienes 50 elementos
...
//
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        FrameLayout posicionemoji,layoutEmojiCreation,fondodescarga;
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            fondodescarga = itemView.findViewById(R.id.fondodescarga);
